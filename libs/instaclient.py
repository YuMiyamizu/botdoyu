...
class InstaClient:
    ...
    def Connect(self):
        if (self.isproxyok != True):
            PrintError("A Proxy não funciona! (Proxy:", self.user, self.ip, ":" ,self.port, ")")
            return

        if (self.ip != None and self.port != None):
            PrintSuccess("Proxy funcionando! (Proxy:", self.user ,self.ip, ":" ,self.port, ")")
        self.GetAndUpdate("https://www.instagram.com/accounts/login/")
        if (self.IsCookiesOK() != True):
            PrintError("Os cookies não foram recebidos! Tente outro proxy! (Proxy:", self.user, self.ip, ":", self.port, ")")
        ...
        if (res.status_code == 200):
            try:
                ...
                if ("message" in obj and obj["message"] == "checkpoint_required"):
                    PrintError("Requer verificação de conta! (URL:", obj["checkpoint_url"], ")")
                    self.isproxyok = False
                    return
                if ("authenticated" in obj and obj["authenticated"] == True and
                "user" in obj and obj["user"] == True):
                    PrintSuccess("Login bem sucedido!", self.user)
                    self.isproxyok = True
                    return
                if ("errors" in obj and "error" in obj["errors"]):
                    PrintError("Falha na autenticação! O proxy pode não estar funcionando. (Proxy:", self.user, self.ip, ":", self.port, ")")
                    self.isproxyok = False
                    return
                self.isproxyok = False
            except:
                PrintError("Falha na autenticação!", self.user)
    ...
    def Spam(self,userid, username, reasonid):
        ...
        try:
            ...
            if ("description" in obj and "status" in obj):
                if (obj["status"] == "ok" and obj["description"] == "Seus relatórios ajudam a manter a comunidade livre de spam."):
                    PrintSuccess("Report enviada com sucesso!", self.user)
                    return
            PrintError("O Pedido de report foi rejeitado!", self.user)
        except:
            PrintError("Ocorreu um erro ao enviar um report!", self.user)
